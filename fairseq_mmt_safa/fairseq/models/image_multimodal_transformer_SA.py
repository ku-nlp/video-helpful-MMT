...
class TransformerEncoder(FairseqEncoder):
    ...
    def __init__(self, args, dictionary, embed_tokens):
        ...
        self.args = args
        # code for image MMT
        if self.args.image_feat_whole_dim[0]>1000:
#             print(self.args.image_feat_whole_dim[0]+"\n\n\n\n\n")
            self.image_feature_compress=nn.Linear(args.image_feat_whole_dim[0], 12)
    ...
    def fuse_img_feat(self, text, idx, image, image_mask, text_mask):
        ...
        if self.args.image_feat_whole_dim[0]>1000:
#             print("image_size before compress:", image.shape) # 1568*B*384
            image=image.permute(1, 2, 0)
#             print("image_size after exchange:", image.shape) # B*384*1568
            image=self.image_feature_compress(image)
#             print("image_size after compress:", image.shape) # B*384*32
            image=image.permute(2, 0, 1)
#             print("image_size after exchange:", image.shape) # 32*B*384
        else:
...
#             raise
        ...
        output, attns = self.selective_attns[idx](query=text, key=image, value=image, key_padding_mask=image_mask)   # t, b, c
        self.attns=attns
        _map = attns.clone().detach()
        ...
        resoft_map = _map[:,:,1:].softmax(dim=-1)
        self.recoder.record_map(_map.cpu(), resoft_map.cpu())
