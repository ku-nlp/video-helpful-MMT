...
def collate(
    ...
    imgs_dist_list = [[] for i in range(len(samples[0]["img_dist_list"]))]
    for s in samples:
        for idx, img in enumerate(s["img_list"]):
            imgs_list[idx].append(img)
    for s in samples:
        for idx, img_dist in enumerate(s["img_dist_list"]):
            imgs_dist_list[idx].append(img_dist)
    for idx, i in enumerate(imgs_list):
        img = torch.stack(i, dim=0)
        img = img.index_select(0, sort_order)
        imgs_list[idx] = img
    for idx, i in enumerate(imgs_dist_list):
        img_dist = torch.stack(i, dim=0)
        img_dist = img_dist.index_select(0, sort_order)
        imgs_dist_list[idx] = img_dist
...
class ImageLanguagePairDataset(FairseqDataset):
    ...
    def __getitem__(self, index):
        ...
        img_dist_item=[i[index][2] for i in self.imgs]
