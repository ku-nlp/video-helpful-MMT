...
class Recorder():   # follow Revisit-MMT
    def __init__(self, args):
        gate_path = os.path.join(args.save_dir, 'gate.pth')
...
#             os.remove(gate_path)
    ...
    def record_map(self, map0, map1):
#         torch.save(map0, os.path.join(self.map_path, str(self.n)+'map.pth'), _use_new_zipfile_serialization=False)
#         torch.save(map1, os.path.join(self.map_path, str(self.n)+'map_origin.pth'), _use_new_zipfile_serialization=False)
        self.n += 1
#         pass

    def record_gate(self, gate, mask):
#         gate = gate.transpose(0, 1).contiguous()
#         torch.save(gate[mask].flatten(), self.gate_path, _use_new_zipfile_serialization=False)
