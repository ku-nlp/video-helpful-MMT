...
#         print("tag 0")
        ...
        # print(f"check 1: src_dataset {len(src_dataset)}")
        if truncate_source:
        ...
        # print(f"check 2: src_dataset {len(src_dataset)}")
...
#     print("tag 1")
    ...
    if prepend_bos:
        ...
        # print(f"check 3: src_dataset {len(src_dataset)}")
        if tgt_dataset is not None:
    ...
    if append_source_id:
        ...
        # print(f"check 4: src_dataset {len(src_dataset)}")
        if tgt_dataset is not None:
...
#         feat_pth_path = os.path.join(image_feat_path, split)
...
#         print("start loading image dataset")
        img_dataset = ImageDataset(feat_pth_path, split, mask_pth_path, image_feat_dim)
        assert len(img_dataset) == len(src_dataset), f"img_dataset:{len(img_dataset)}, src_dataset:{len(src_dataset)}"
...
class imageMMTTask(LegacyFairseqTask):
    ...
    def add_args(parser):
        ...
        parser.add_argument('--image-feat-whole-dim', nargs='+', type=int,
                            help='image features dimension')
    ...
    def load_dataset(self, split, epoch=1, combine=False, **kwargs):
        ...
        self.datasets[split] = load_langpair_dataset(
            ...
            image_feat_dim=self.args.image_feat_whole_dim
        )
    '''
