...
#         logger.warning("Start train_step")
        now=time.time()
#         logger.warning("Start train_step")
...
#         logger.warning("Start enumerate samples")
        for i, sample in enumerate(samples):
            sample = self._prepare_sample(sample)
            
#             logger.warning(f"End of prepare_sample: {time.time()-now}")
            now=time.time()
...
#             logger.warning("Start maybe_no_sync")

#             logger.warning(f"End of part1: {time.time()-now}")
            now=time.time()
...
#             logger.warning(f"End of part2: {time.time()-now}")
            now=time.time()
...
#         logger.warning("Start _sync_statsb")
        # gather logging outputs from all replicas
    
#         logger.warning(f"End of part3: {time.time()-now}")
        now=time.time()
        
        if self._sync_stats():
#             logger.warning("Start in _sync_statsb")
...
#             logger.warning("Start all_reduce")
            self.model.all_reduce()
        
#         logger.warning(f"End of part4: {time.time()-now}")
        now=time.time()
...
#                 logger.warning("Start multiply-grads")
...
#                 logger.warning("Start optimizer")
...
#         logger.warning(f"End of part5: {time.time()-now}")
        now=time.time()
        
        # Some distributed wrappers (e.g., SlowMo) need access to the optimizer after the step
        if hasattr(self.model, "perform_additional_optimizer_actions"):
#             logger.warning("Start perform_additional_optimizer_actions")
...
#         logger.warning(f"End of part6: {time.time()-now}")
        now=time.time()
        
        if not overflow or self.args.distributed_wrapper == "SlowMo":
#             logger.warning("Start SlowMo")
...
#         logger.warning(f"End of part7: {time.time()-now}")
        now=time.time()
...
#         logger.warning("End of train_step")
