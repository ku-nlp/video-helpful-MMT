...
def last_n_checkpoints(paths, n, update_based, upper_bound=None, use_best=0):
    assert len(paths) == 1
    path = paths[0]
    if update_based:
        pt_regexp = re.compile(r"checkpoint_\d+_(\d+)\.pt")
    elif use_best:
        pt_regexp = re.compile(r"checkpoint.best_bleu_(\d+)\.(\d+)\.pt") # checkpoint.best_bleu_10.38.pt
    else:
    ...
    if not use_best:
        return [os.path.join(path, x[1]) for x in sorted(entries, reverse=False)[:n]] # originally True
    else:
...
def main():
    ...
    parser.add_argument('--use-best', type=int,
                        help='use best n instead of last n'
                        )
    # fmt: on
