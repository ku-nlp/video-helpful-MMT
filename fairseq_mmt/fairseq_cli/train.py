...
#     print("Finish task setup", time.time()-now)
#     now=time.time()
...
#     print("Finish load validation", time.time()-now)
#     now=time.time()

    # Build model and criterion
    model = task.build_model(args)
    criterion = task.build_criterion(args)
#     print("Finish build model", time.time()-now)
#     now=time.time()
    ...
    while lr > args.min_lr and epoch_itr.next_epoch_idx <= max_epoch:
        model.current_epoch=epoch_itr.next_epoch_idx-1
        # train for one epoch
    ...
    for i, samples in enumerate(progress):
        ...
        if log_output is not None:  # not OOM, overflow, ...
            ...
            if num_updates % args.log_interval == 0:
                ...
                logger.info(f"Last {args.log_interval} batches cost {spent_time/60} min")
